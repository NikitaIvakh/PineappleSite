...
namespace Identity.Infrastructure.Services
{
    public class AuthService(UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, IOptions<JwtSettings> jwtSettings,
        IAuthRequestDroValidator validationRules, IRegisterRequestDtoValidator validationRules1) : IAuthService
    {
        private readonly UserManager<ApplicationUser> _userManager = userManager;
        private readonly SignInManager<ApplicationUser> _signInManager = signInManager;
        private readonly JwtSettings _jwtSettings = jwtSettings.Value;
        private readonly IAuthRequestDroValidator _authValidator = validationRules;
        private readonly IRegisterRequestDtoValidator _registerValidator = validationRules1;

        public async Task<BaseIdentityResponse<AuthResponse>> LoginAsync(AuthRequest authRequest)
        {
            var response = new BaseIdentityResponse<AuthResponse>();
        ...
        public async Task<BaseIdentityResponse<RegisterResponse>> RegisterAsync(RegisterRequest registerRequest)
        {
            var response = new BaseIdentityResponse<RegisterResponse>();

            try
            {
                var validatorREsult = await _registerValidator.ValidateAsync(registerRequest);
