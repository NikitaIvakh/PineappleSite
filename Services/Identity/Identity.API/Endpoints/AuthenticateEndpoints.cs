...
public sealed class AuthenticateEndpoints : ICarterModule
{
    ...
    private static async Task<Results<Ok<Result<string>>, BadRequest<string>>> Login(ISender sender,
        ILogger<AuthRequestDto> logger, [FromBody] AuthRequestDto authRequestDto)
    {
        ...
        logger.LogError($"LogDebugError ================ Войти в аккаунт не удалось: {authRequestDto.EmailAddress}");
    ...
    private static async Task<Results<Ok<Result<string>>, BadRequest<string>>> Register(ISender sender,
        ILogger<RegisterRequestDto> logger, [FromBody] RegisterRequestDto registerRequestDto)
    {
        ...
        logger.LogError($"LogDebugError ================ Регистрация не удалась: {registerRequestDto.EmailAddress}");
        return TypedResults.BadRequest(string.Join(", ", command.ValidationErrors!));
    }
    ...
    private static async Task<Results<Ok<Result<Unit>>, BadRequest<string>>> RevokeToken(ISender sender,
        ILogger<string> logger, string userName)
    {
        var command = await sender.Send(new RevokeTokenRequest(userName));
    ...
    private static async Task<Results<Ok<CollectionResult<Unit>>, BadRequest<string>>> RevokeTokens(ISender sender,
        ILogger<Unit> logger)
    {
