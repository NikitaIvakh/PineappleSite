    ...
    : IUserRepository
{
    ...
    public async Task<IdentityResult> RemoveFromRolesAsync(ApplicationUser user, CancellationToken token = default)
    {
        if (user is null)
        {
            throw new ArgumentNullException(nameof(user), "Объект не найден");
        }
